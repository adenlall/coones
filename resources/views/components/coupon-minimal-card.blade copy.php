<div id="card_{{$id}}_p" class="flex flex-col justify-between items-center shadow-md shadow-black/30 rounded-md bg-base-100 p-3 space-y-2 w-full">
    <div class="w-full space-y-2">
        <div class="flex flex-row items-center justify-between w-full">
            <a href="{{'/coupons/'.$store->_store_name}}"><img class="w-[110px] h-[58px] rounded-md" src="{{$store->thumbnail}}"/></a>
    ...
    <x-share-modal :id="$id" :title="$title" :url="urlencode('http://localhost/coupons/'.$store->_store_name)" />
    ...
    <div class="flex justify-between items-center w-full">
        ...
        <a href="/coupons/{{$store->_store_name}}"><p class="text-sm underline underline-offset-2">زيارة المتجر</p></a>
    </div>
