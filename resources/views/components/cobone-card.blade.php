    ...
    <div class="flex flex-col justify-between items-center p-3 space-y-2 w-full h-full">
        <div class="w-full space-y-2">
            <div class="flex flex-row items-center justify-between w-full">
                <meta content="متجر {{$store->_store_name??$store->_store_param}}" />
                ...
                <a aria-label="زيارة العرض" href="{{'/coupons/'.($store->_store_param===null?$store->_store_name:$store->_store_param)}}">
                    <img class="w-[135px] h-[68px] rounded-md" alt="شعار متجر {{$store->_store_name??$store->_store_param}}" src="{{$store->thumbnail}}"/>
                </a>
        ...
        <div class="w-[100%] space-y-2">
            <a aria-label="{{$store->_store_name??$store->_store_param}}" href="/coupons/{{$store->_store_param===null?$store->_store_name:$store->_store_param}}" class="btn rounded-sm btn-sm btn-ghost font-bold text-lg w-full mt-3">عرض جميع الكوبونات</a>
