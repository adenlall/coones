            ...
            <ul class="flex-center-col gap-2">
                @foreach($latest as $index => $item)
                    <li itemscope itemtype="https://schema.org/Organization" itemprop="itemListElement" class="w-full">
                        ...
                        <a aria-label="{{$item->_store_name}}" itemprop="url" class="block bg-accent text-center font-medium w-full rounded-md text-accent-content p-1" href="/store/{{$item->_store_name}}">
                            {{$item->_store_name}}
                        </a>
                    </li>
                @endforeach
