<x-layout>
    ...
    <div class="w-full bg-base-100 mb-6">
        ...
        <div class="ccontainer m-auto w-full">
        <ul id="offerlist" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 items-stretch justify-center gap-5 md:px-0 px-2">
            @foreach($stores as $store)
                <li class="w-full h-auto bg-base-100 rounded-md shadow-md shadow-black/30">
                    <a href="/store/{{$store->_store_name}}" class="block p-2 w-full h-full">
                        <div class="my-5">
                            <img class="w-[135px] h-[68px] rounded-md m-auto" src="{{$store->thumbnail}}" alt="شعار متجر {{$store->_store_name}}"/>
                        </div>
                        <hr/>
